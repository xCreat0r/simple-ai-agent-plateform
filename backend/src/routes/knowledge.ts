...
const knowledgeRoutes = new Hono<{ Bindings: CloudflareEnv }>();
...
knowledgeRoutes.post("/", async (c) => {
  ...
  await getDb().insert(knowledgeBases).values({ id: kbId, userId: user.id, name, createdAt: new Date() });
  return c.json({ id: kbId, userId: user.id, name }, 201);
});
